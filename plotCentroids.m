function plotCentroids(glob, depos, modelName)

    centroidsCalculated = isfield(glob, 'centroidX');
    
    if centroidsCalculated

        ffOne = figure;
        
        for t = 2:glob.totalIterations                      
             yco = [glob.centroidY(t)-0.5 glob.centroidY(t)-0.5 glob.centroidY(t)+0.5 glob.centroidY(t)+0.5] * glob.dy;  % 4 vertices coordinates clockwise 
             xco = [glob.centroidX(t)-0.5 glob.centroidX(t)+0.5 glob.centroidX(t)+0.5 glob.centroidX(t)-0.5] * glob.dx;
             zco = [t t t t];

             patch(xco,yco,zco,depos.flowColours(t,:));
        end
        ...
        % set(gca,'Color','none');
    %     fName = strcat('graphicsOutput/', modelName, '_centroids3DPlot');
    %     export_fig(fName, '-png', '-transparent', '-m12', '-q101');
    end
end