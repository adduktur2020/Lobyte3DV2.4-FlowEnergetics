function [depos, topog, erosionMap, flowSedVolume, flowSedConcentration, erodeThick, flowCellTime, flowErosionRate, topogErode] = calcFlowErosionIBL(glob, depos, flow, topog, erosionMap, xco, yco)
   ...
   if flow.velocity > glob.deposVelocity && ~flow.stopped % Flow is moving and velocity is greater than settling velocity threshold, so erode
       ...
       flowCellTime = abs((glob.dx) / flow.velocity); % (s) Residence time in each cell along x-axis
        ...
        if erosionMap(yco, xco) > 0
            [depos, topog(yco,xco), erosionMap(yco, xco)] = calcErodeStrataIB(glob, depos, topog, erosionMap, xco, yco);
        end
        
        erodeThick = (erosionMap(yco,xco) * glob.dx * glob.dy); %added by IB
        topogErode = topog(yco,xco);
